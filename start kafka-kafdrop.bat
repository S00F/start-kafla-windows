...
wt.exe new-tab --title "zookeeper" cmd /k "cd /d %KAFKA_FOLDER%\bin\windows && zookeeper-server-start.bat ..\..\config\zookeeper.properties"  

:: Wait for specified seconds
timeout /t %DELAY% /nobreak

:: Open a second tab for the second script
wt.exe new-tab --title "kafka" cmd /k "cd /d %KAFKA_FOLDER%\bin\windows && kafka-server-start.bat ..\..\config\server.properties"
